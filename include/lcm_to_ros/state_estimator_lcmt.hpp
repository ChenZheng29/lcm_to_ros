...
int state_estimator_lcmt::_encodeNoHash(void *buf, int offset, int maxlen) const
{
    ...
    tlen = __float_encode_array(buf, offset + pos, maxlen - pos, &this->pz[0], 4);
    if(tlen < 0) return tlen; else pos += tlen;
...
int state_estimator_lcmt::_decodeNoHash(const void *buf, int offset, int maxlen)
{
    ...
    tlen = __float_decode_array(buf, offset + pos, maxlen - pos, &this->pz[0], 4);
    if(tlen < 0) return tlen; else pos += tlen;
...
{
    uint64_t hash = 0xe2ce94e60fffc44fLL;
